...
- **P1-06 — Initial scan and reconciliation engine — Ready, next.** Index existing files, detect additions/removals/renames, recover from interruptions, and report conflicts without deleting unknown data. Volume health gates every removal; see [ADR 0007](../adr/0007-reconciliation-and-mount-health.md). `P1-04` left it the tables, the recorded root identity and the marker file to verify them against, and left it the coverage gap it exists to close: content predating the index, and the descendants of a restored folder, are unindexed until the scan runs. Also owns orphaned trash records and trash retention, which `P1-12` deliberately left out because expiry needs a scheduled job.
