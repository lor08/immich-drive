...
- **P0-04 — Migration architecture — Ready.** Define in-place upgrade, side-by-side fallback, compatibility matrix, backup preflight, rollback boundaries, and real-fixture testing. Tracked by Issue #9.
- **P0-05 — Release and publication architecture — Ready.** Define fork-owned image names, registries, tags, package visibility, signing identities, artifact retention, Android delivery, and iOS/TestFlight delivery. Tracked by Issue #11.
...
- **P1-03 — Storage-root configuration and validation — Active.** Add configuration, startup validation, non-overlap checks against Immich upload/library paths, mount readiness, permissions checks, and operator-facing errors. The domain is opt-in: with `IMMICH_DRIVE_ROOT` unset the server behaves exactly like upstream Immich. Tracked by Issue #22.
