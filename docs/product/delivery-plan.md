...
- **P1-04 — Drive-owned database schema — Done.** `drive_volume` and `drive_entry`, declared in fork-owned files, created by a migration carrying a fixed `9000000000000` prefix so a Drive migration always sorts after anything upstream adds and can never become unordered; see [ADR 0011](../adr/0011-drive-index-schema.md). Every mutation maintains the index and a failed index write never fails the operation. Two departures from this line as originally written: trash manifests are **not** migrated into a table, because that would give a deleted entry two sources of truth, and checksums stay with `P1-13`. Nothing reads the index yet. Tracked by Issue #56, task file `docs/tasks/0026-drive-index-schema.md`.
...
- **P1-06 — Initial scan and reconciliation engine — Done.** Operator-triggered passes over a volume: entries found on disk are indexed, rows whose file is gone are marked `missing`, disagreements are marked `conflicted`, and nothing is deleted. Health is evaluated first and gates every conclusion — an empty tree against a populated index refuses rather than reading as a mass deletion; see [ADR 0007](../adr/0007-reconciliation-and-mount-health.md). Passes are checkpointed and resumable. Two departures from this line as written: the trash is _reported_ rather than given index rows, for the reason [ADR 0011](../adr/0011-drive-index-schema.md) gives, and scheduled runs stay out because they need the job-queue seam. Retention is opt-in via `IMMICH_DRIVE_TRASH_RETENTION_DAYS`. Tracked by Issue #57, task file `docs/tasks/0027-reconciliation-and-health.md`.
- **P1-07 — Ownership and authorization model — In review.** The shared volume gains an explicit member list with read-only or read-write access, and every file operation resolves through one decision point that a new entry point cannot bypass — `FileDomainService` no longer holds the volume registry at all. Membership is the first Drive state that cannot be rebuilt from the filesystem, so it is argued in [ADR 0012](../adr/0012-shared-volume-membership.md) and deliberately kept out of the index tables: an empty member list means nobody, and rebuilding the cache does not revoke anyone. Private volumes are unchanged and need no row. Per-path sharing and share links remain Phase 7. Tracked by Issue #64, task file `docs/tasks/0029-shared-volume-authorization.md`.
...
- **P1-20 — Scheduled reconciliation — In review.** Takes the job-queue seam `P1-06` left alone: two `JobName`s on the existing `BackgroundTask` queue, a handler registered in the upstream services registry because job discovery walks that list, and the file domain imported into the microservices worker. The schedule is `IMMICH_DRIVE_RECONCILE_CRON`, unset by default so no deployment inherits background filesystem scans, validated at startup, owned by one replica through a database lock. A volume larger than one pass is finished by a chain of jobs that stops when it stops making progress. Not included: Drive settings in the admin UI, and therefore a schedule editable without a restart. Tracked by Issue #62, task file `docs/tasks/0028-scheduled-reconciliation.md`.
...
- **P1-13 — Checksums and integrity state — In review.** `sha256` computed in the same pass that writes a file, stored with its algorithm beside it, and used where it decides something: a modification-time-only disagreement is settled by reading the bytes, so a `touch` or a metadata-preserving copy **recovers** a row instead of conflicting it forever, while a real edit is conflicted on evidence. A size difference still needs no read. Backfilling existing content is opt-in and bounded by `IMMICH_DRIVE_CHECKSUM_BUDGET_MB`, so no deployment inherits a full-volume read. Duplicate hints and repair flows are deliberately not included. Tracked by Issue #66, task file `docs/tasks/0030-checksums.md`.
