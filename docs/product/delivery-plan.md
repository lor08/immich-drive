...
- **P1-08 — Volume discovery and module registration — Active.** One authenticated endpoint listing the caller's volumes, plus the module registration that serves it. This is the first upstream seam the fork takes on, recorded in the seam inventory. Tracked by Issue #28.
- **P1-18 — Folder listing and creation API — Ready.** List folder contents and create directories atomically, with naming rules and generated clients. Split out of `P1-08` so path handling and write semantics get their own review.
