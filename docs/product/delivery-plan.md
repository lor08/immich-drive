...
- **P0-15 — Offline Dart template generation — Active.** Generate from the committed mustache templates instead of re-downloading them on every run, after a fetch failure broke a required check with no fault in the code. Tracked by Issue #48.
- **P0-16 — Diagnosable e2e database startup — Ready.** Capture the database container's logs when the end-to-end setup fails, identify why connections are refused after `docker compose up --wait`, then fix that cause. Observability first, because the failure is intermittent and a fix aimed at a guess would look like it worked. Tracked by Issue #49.
