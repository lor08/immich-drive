...
- **P0-14 — Runnable inherited validation — Active.** Three inherited jobs depend on the upstream `mich` runner or a DCM licence; `Lint Web` and the Android build queue forever, and Dart analysis fails before it starts. Degrade them behind `github.repository_owner`, keeping the free analysis and moving web linting to a hosted runner. Tracked by Issue #30.
...
- **P1-08 — Folder query and creation API — Backlog.** List volumes and folders, create directories atomically, enforce naming rules, and generate OpenAPI clients. This task also registers the module in `app.module.ts`, which is the first upstream seam the fork takes on, and records it in the seam inventory.
...
- **P1-16 — Volume and path namespace model — Active.** Implement the volume abstraction from [ADR 0004](../adr/0004-volume-path-model.md): a private volume per user, one configuration-defined shared space, service directories outside the browsable tree, per-volume root identity, and an API addressed by volume plus relative path. The registry is configuration-driven until `P1-04` moves it into the schema. Tracked by Issue #26.
