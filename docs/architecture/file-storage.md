...
`IMMICH_DRIVE_CHECKSUM_BUDGET_MB` is how many megabytes one reconciliation pass may read purely to give existing files a checksum. **Unset means none**, which is the default: files without a digest work exactly as before, and reading a whole volume is not something a deployment should inherit by upgrading. Digests are otherwise computed only where they decide something — in the same pass that writes a file, and when a file's modification time moved while its size did not, which is the difference between a `touch` and an edit.
