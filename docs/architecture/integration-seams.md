...
| ---------------------------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `server/src/app.module.ts`                                             | `P1-08`, `P1-20` | Registers `FilesModule` in `ApiModule`, and — since `P1-20` — in `MicroservicesModule` and `ImmichAdminModule` as well: all three build from the same provider list, so each instantiates the Drive job handler and needs the file domain resolvable.                                                                                          |
| `server/src/enum.ts`                                                   | `P1-08`          | `ApiTag.Files` and the `file.*` permissions, appended to existing enums. Every later slice that needs a permission appends one line here.                                                                                                                                                                                                      |
| `server/src/constants.ts`                                              | `P1-08`          | `endpointTags` entry, required by the `Record<ApiTag, string>` type.                                                                                                                                                                                                                                                                           |
| `.github/workflows/test.yml`                                           | `P0-14`, `P0-16` | `Lint Web` picks a GitHub-hosted runner outside upstream; the `mich` runner does not exist here, so the job queued forever. `P0-16` also retries the end-to-end stack startup and stops the suites running when it never came up.                                                                                                              |
...
| `server/src/schema/index.ts`                                           | `P1-04`          | Two imports, two entries in `ImmichDatabase.tables`, two keys in the `DB` interface. A Drive-owned table has to be declared here or `SQL Schema Checks` treats it as schema drift and `sql-tools` generates a migration to drop it.                                                                                                            |
| `server/src/types.ts`                                                  | `P1-20`          | Two entries in the `JobItem` union and one job data interface. `jobRepository.queue()` takes `JobItem`, which is exhaustive, so a Drive job cannot be queued without appearing there.                                                                                                                                                          |
| `server/src/services/index.ts`                                         | `P1-20`          | One import and one array entry, registering the Drive job handler. Job discovery walks this list, and `JobRepository.setup` throws `ImmichStartupError` for a `JobName` with no handler — so adding a job at all forces this edit.                                                                                                             |
| `server/src/schema/migrations/`                                        | `P1-04`          | Not an edit but a shared namespace: Drive migrations live in the upstream directory and are ordered against upstream's by filename. They therefore carry a fixed `9000000000000` prefix so they always sort last and can never become unordered; see [ADR 0011](../adr/0011-drive-index-schema.md). Never edit an existing upstream migration. |
...
- one schema file, from owning tables at all (`P1-04`), which the spike deliberately did not do — [ADR 0005](../adr/0005-defer-drive-database.md) deferred the database until the filesystem-only slices were delivered;
- two files from running work in the background (`P1-20`): a job cannot be queued without an entry in the exhaustive `JobItem` union, and its handler cannot be discovered without an entry in the services registry — which the server enforces by refusing to start.
